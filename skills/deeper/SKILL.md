...
You are the LAUNCHER and the orchestrator — the main Claude session runs the round loop directly, one round per turn, dispatching a fresh Explore subagent per round for Q and another for A. You do NOT reason about the user's claim — that judgment belongs to the per-round Explore subagents. You set up state, arm Monitor for live progress, run the round handler, surface what arrives. Nothing more.
...
1. **Determine the mode and seed claim.**
   - **Default — auto.** If the invocation is `/deeper <claim>` or `/deeper auto <claim>`, set `MODE=auto` and seed = the claim text. The main session runs the round loop directly, one round per turn, dispatching fresh Explore subagents per round for Q and A.
...
3. **Dispatch.**
   - `MODE=auto`: go to **## Main-session orchestrator** below. In Turn 1:
     ...
     (c) end the turn. Subsequent rounds run in subsequent turns, woken by judge_result Monitor notifications. See the section below for the full protocol.
...
The main Claude session runs the round loop itself, one round per turn. Q and A both come from fresh Explore subagents — the launcher never produces content.

Why one round per turn (not all rounds in a single turn): focus mode shows only the last text message of a turn, and Monitor's stream is only visible to the user across turn boundaries. Multi-turn lets each round's events surface live, and lets the user interrupt at any round boundary by sending a message.

### Turn 1 (launcher entry)
...
2. **Arm Monitor automatically** — do NOT ask the user, do NOT skip:
   ...
   Capture the task id — you will TaskStop it at run end. Persist it to `$RUN_DIR/.monitor-task-id` so the wake handler can recover it across turns.

3. Tell the user one line: `Run started: $RUN_ID (auto, cap=$CAP). Streaming progress below.`

4. Run the **Round handler** below for N=1.

5. End the turn. The judge_result Monitor notification will wake the launcher into round 2.

### Round handler (one round, one turn)
...
6. **End the turn.** Do NOT loop into round N+1 in the same turn — let the judge_result event surface to the user via Monitor first. The launcher will wake automatically on the judge_result notification.

### Wake handler (turns after Turn 1)

Each subsequent turn is woken by a Monitor notification (or, if the run is already done, by an explicit user message). On wake:

1. Read the **last line of `$RUN_DIR/events.jsonl`**. Parse JSON.

2. Branch on `type`:
   - `question_emitted` | `answer_emitted` | `stall`: do nothing. The Monitor already surfaced these to the user. End the turn — wait for the next notification.
   - `judge_result`:
     - If `detail.done == true` → go to "## After completion (exit handler)".
     - Else: determine current round N from `max(round in events.jsonl)`. If N >= CAP (default 8): go to "## After completion" with status `auto_cap`. Otherwise: run "### Round handler" for round N+1, then end the turn.
   - `run_finished`: terminal event already emitted — go to "## After completion" (idempotent close-out).

3. **If the wake source is a user message** (not a Monitor notification) mid-drill: treat it as an interrupt. Go to "## After completion" with status `aborted` (reason: `user_interrupt`). Acknowledge the interrupt in one line.
...
- If `MODE=auto`: arm Monitor (per "## Main-session orchestrator", Turn 1, step 2) and run the Round handler for the next pending round N. The tree.json `cursor` and events.jsonl max-round determine where to resume.
...
**Auto mode**: if N reaches `DEEPER_AUTO_CAP` (default 8) without an exit, the wake handler routes to "## After completion (exit handler)" with status `auto_cap`. The exit handler renders the tree + dispatch chain and offers next actions via `AskUserQuestion`.
...
| "I'll dispatch Q and A in the same turn AND start round N+1 immediately" | NO. End the turn after judge.sh. Multi-turn round structure is required so Monitor's stream is visible to the user (focus mode constraint). |
...
| "Let me skip Monitor — it's just decoration" | NO. Monitor is the user-visible progress channel. Without it, focus mode shows nothing until completion. Arm it in Turn 1 unconditionally. |
